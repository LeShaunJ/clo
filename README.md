...
```sh
clo [--model MODEL] [--inst URL] [--db NAME] [--user NAME] [--out FILE] [--env FILE] [--log LEVEL] [--dry-run] [--help] [--version] ACTION ...
```
...
| `‑‑count` |  | NO | If `True`, only counts and returns the number of matching records. | `false` |
